 ...
 * @group Events
 */
export interface AddEventSessionPassTypeParams extends MutationParams {
  eventId: string;
  sessionId: string;
  passTypeId: string;
}
 ...
 * @group Events
 */
export const AddEventSessionPassType = async ({
  eventId,
  sessionId,
  passTypeId,
  adminApiParams,
  queryClient,
}: AddEventSessionPassTypeParams): Promise<
 ...
 * @group Events
 */
export const useAddEventSessionPassType = (
  options: Omit<
    ConnectedXMMutationOptions<
      Awaited<ReturnType<typeof AddEventSessionPassType>>,
      Omit<AddEventSessionPassTypeParams, "queryClient" | "adminApiParams">
    >,
    "mutationFn"
  > = {}
) => {
