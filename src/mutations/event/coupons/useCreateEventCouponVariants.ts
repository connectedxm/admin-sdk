...
export const CreateEventCouponVariants = async ({
  ...
  adminApiParams,
  queryClient,
}: CreateEventCouponVariantsParams): Promise<ConnectedXMResponse<Coupon[]>> => {
  ...
  if (queryClient && data.status === "ok") {
    ...
    // Cache each created coupon variant individually
    data.data.forEach((coupon) => {
      SET_EVENT_COUPON_QUERY_DATA(queryClient, [eventId, coupon.id], {
        ...data,
        data: coupon,
      });
    });
